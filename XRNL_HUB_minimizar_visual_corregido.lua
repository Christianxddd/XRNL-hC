...
Tabs.ESP:AddSlider({
    ...
    Description = "Ajusta qué tan alto puedes saltar",
    Default = 50,
    Minimum = 50,
    Maximum = 200,
    Callback = function(value)
...
-- 🫥 Invisibilidad (modo simple)
Tabs.ESP:AddButton({
    Title = "Activar Invisibilidad",
    Description = "Hace invisible tu personaje (método básico)",
    Callback = function()
...
-- 🛡️ Antiban (básico - placebo)
Tabs.ESP:AddButton({
    Title = "Activar Antiban (Placebo)",
    Description = "Evita detecciones comunes (puede no funcionar en todos los juegos)",
    Callback = function()
        setreadonly(hookmetamethod or setmetatable, false)
        -- Este es solo un ejemplo básico, no garantiza protección
    end
})

-- 🌀 Antifling
Tabs.ESP:AddButton({
    Title = "Activar Antifling",
    Description = "Evita que otros jugadores te lancen por el mapa",
    Callback = function()
...
-- ✅ REEMPLAZAMOS el botón original de minimizar para que no destruya el GUI
task.spawn(function()
    local CoreGui = game:GetService("CoreGui")
    local Rayfield = CoreGui:WaitForChild("Rayfield", 10)

    if Rayfield then
        local Topbar = Rayfield:FindFirstChild("Topbar", true)
        if Topbar then
            local OriginalMinimize = Topbar:FindFirstChild("Minimize")
            if OriginalMinimize then
                OriginalMinimize:Destroy()
            end

            -- Creamos nuevo botón personalizado
            local NewMinimize = Instance.new("ImageButton")
            NewMinimize.Name = "Minimize"
            NewMinimize.Size = UDim2.new(0, 20, 0, 20)
            NewMinimize.Position = UDim2.new(1, -60, 0, 10)
            NewMinimize.BackgroundTransparency = 1
            NewMinimize.Image = "rbxassetid://7733964644"
            NewMinimize.Parent = Topbar

            local originalSize = Rayfield.Size
            local originalPosition = Rayfield.Position
            local minimizedSize = UDim2.new(0, 300, 0, 45)
            local minimizedPosition = UDim2.new(0, 20, 0, 20)
            local minimized = false

            -- Toggle de tamaño
            NewMinimize.MouseButton1Click:Connect(function()
                if minimized == false then
                    Rayfield.Size = minimizedSize
                    Rayfield.Position = minimizedPosition
                    minimized = true
                else
                    Rayfield.Size = originalSize
                    Rayfield.Position = originalPosition
                    minimized = false
                end
            end)
        end
