...
-- 📱 Botón flotante pequeño para KRNL Mobile que restaura el panel si desaparece
task.spawn(function()
    local CoreGui = game:GetService("CoreGui")

    local RestoreButton = Instance.new("ImageButton")
    RestoreButton.Name = "XRNLRestoreButton"
    RestoreButton.Size = UDim2.new(0, 40, 0, 40)
    RestoreButton.Position = UDim2.new(0, 10, 0, 100)
    RestoreButton.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    RestoreButton.BackgroundTransparency = 0.4
    RestoreButton.BorderSizePixel = 0
    RestoreButton.Image = "rbxassetid://120008128829681"
    ...
    RestoreButton.Parent = CoreGui

    -- Si el panel desaparece, mostramos el botón
    task.spawn(function()
        while true do
            task.wait(1)
            local rf = CoreGui:FindFirstChild("Rayfield")
            if not rf or not rf.Enabled then
    ...
    -- Al presionar el botón, reactivamos el panel
    RestoreButton.MouseButton1Click:Connect(function()
        local rf = CoreGui:FindFirstChild("Rayfield")
        if rf then
            rf.Enabled = true
            rf.Size = UDim2.new(0, 600, 0, 400) -- restaurar tamaño estándar
            rf.Position = UDim2.new(0.5, -300, 0.5, -200)
        end
    end)
end)
